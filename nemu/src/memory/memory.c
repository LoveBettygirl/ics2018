...
paddr_t page_translate(vaddr_t addr, bool is_write) {
  if (cpu.cr0.paging) {
    paddr_t pdbase = ((uint32_t)cpu.cr3.page_directory_base << 12) | (((addr >> 22) & 0x3ff) << 2);
    PDE pde;
    pde.val = paddr_read(pdbase, sizeof(PDE));
    assert(pde.present);
    paddr_t ptbase = ((uint32_t)pde.page_frame << 12) | (((addr >> 12) & 0x3ff) << 2);
    PTE pte;
    pte.val = paddr_read(ptbase, sizeof(PTE));
    assert(pte.present);
    paddr_t paddr = ((uint32_t)pte.page_frame << 12) | (addr & 0xfff);
    pde.accessed = 1;
