...
paddr_t page_translate(vaddr_t addr, bool is_write) {
  if (cpu.cr0.paging) {
    paddr_t pdbase = cpu.cr3.page_directory_base;
    pdbase = (pdbase << 12) | (((addr >> 22) & 0x3ff) << 2);
    PDE pde;
    pde.val = paddr_read(pdbase, sizeof(PDE));
    assert(pde.present);
    paddr_t ptbase = pde.page_frame;
    ptbase = (ptbase << 12) | (((addr >> 12) & 0x3ff) << 2);
    PTE pte;
    pte.val = paddr_read(ptbase, sizeof(PTE));
    assert(pte.present);
    paddr_t paddr = pte.page_frame;
...
uint32_t vaddr_read(vaddr_t addr, int len) {
  if (data_cross_page(addr, len)) {
    ...
    data = (paddr_read(high_paddr, len - low_len) << (low_len << 3)) | data;
    return data;
  }
  else {
...
void vaddr_write(vaddr_t addr, int len, uint32_t data) {
  if (data_cross_page(addr, len)) {
    ...
    paddr_write(low_paddr, low_len, (data << (high_len << 3) >> (high_len << 3)));
    paddr_t high_paddr = page_translate(high_vaddr, true);
    paddr_write(high_paddr, high_len, data >> (low_len << 3));
  }
  else {
